...
export default function MusCard({
  ...
  if (musDeckTheme === "classic") {
    return (
      <motion.button type="button" onClick={onClick} disabled={!onClick}
        initial={{ opacity: 0, scale: 0.85 }} animate={{ opacity: dimmed ? 0.4 : 1, scale: 1, y: selected ? -12 : 0 }}
        transition={{ duration: dur, delay, ease: [0.22, 1, 0.36, 1] }}
        className={`${sizeClass} relative rounded-md border bg-[#fbfaf6] flex flex-col justify-between select-none transition-colors ${selected ? "border-accent ring-2 ring-accent" : "border-black/15"} ${showCardShadow ? "shadow-sm" : ""} ${onClick ? "cursor-pointer" : "cursor-default"}`}
        style={{ padding: mini ? 2 : small ? 3 : 4 }} title={`${RANK_SHORT[card.rank]} de ${SUIT_NAME[card.suit]}`}>
        ...
        <div className="flex items-center justify-center flex-1 pointer-events-none"><SpanishSuit suit={card.suit} size={mini ? 20 : small ? 30 : 46} /></div>
